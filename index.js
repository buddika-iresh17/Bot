...
const settingsMap = { "1": { key: "MODE", trueVal: "private", falseVal: "public", label: "Bot Mode" }, "2": { key: "AUTO_REACT", trueVal: "true", falseVal: "false", label: "Auto-React" }, "3": { key: "AUTO_READ_STATUS", trueVal: "true", falseVal: "false", label: "Auto-Read-Status" }, "4": { key: "AUTO_STATUS_REPLY", trueVal: "true", falseVal: "false", label: "Auto-Status-Reply" }, "5": { key: "AUTOLIKESTATUS", trueVal: "true", falseVal: "false", label: "Auto-like-status" }, "6": { key: "READ_MESSAGE", trueVal: "true", falseVal: "false", label: "Read-message" }, "7": { key: "ANTI_LINK", trueVal: "true", falseVal: "false", label: "Anti-link" }, "8": { key: "ANTI_LINK_KICK", trueVal: "true", falseVal: "false", label: "Anti-link-kick" }, "9": { key: "ANTI_DEL_PATH", label: "Anti-delete Path", customOptions: ["log", "chat", "inbox"] }, "10": { key: "ANTIDELETE", trueVal: "true", falseVal: "false", label: "Anti-Delete" } };

if (command === "settings" || command === "config") { try { const senderNumber = m.sender.split("@")[0]; const botOwner = getBotOwner(conn); if (senderNumber !== botOwner) return reply("📛 Only the bot owner can use this command!");

const sentMsg = await conn.sendMessage(from, {
  image: { url: config.ALIVE_IMG },
  caption:
    `╔═══❍ *BOT SETTINGS* ❍═══⫸\n` +
    `╠➢ 1. Bot Mode (Private / Public)\n` +
    `╠➢ 2. Auto React (On / Off)\n` +
    `╠➢ 3. Auto Read Status (On / Off)\n` +
    `╠➢ 4. Auto Status Reply (On / Off)\n` +
    `╠➢ 5. Auto Status Like (On / Off)\n` +
    `╠➢ 6. Read Message (On / Off)\n` +
    `╠➢ 7. Anti-Link (On / Off)\n` +
    `╠➢ 8. Anti-Link Kick (On / Off)\n` +
    `╠➢ 9. Anti-Delete Path (Log / Chat / Inbox)\n` +
    `╠➢ 10. Anti-Delete (On / Off)\n` +
...
const menuListener = async (msgData) => {
  try {
    ...
    const isReply = msg?.extendedTextMessage?.contextInfo?.stanzaId === menuMessageID;
    const text = msg?.conversation || msg?.extendedTextMessage?.text;

    if (!isReply || sender !== botOwner || !text) return;

    const selected = text.trim();
    const setting = settingsMap[selected];
    if (!setting) return reply("❌ Invalid option. Please reply with a number between 1 and 10.");
    ...
    const toggleListener = async (msgData2) => {
      try {
        ...
        const msg2 = received2.message;
        const sender2 = (received2.key.participant || received2.key.remoteJid).split("@")[0];
        const isReply2 = msg2?.extendedTextMessage?.contextInfo?.stanzaId === toggleID;
        const text2 = msg2?.conversation || msg2?.extendedTextMessage?.text;
        if (!isReply2 || sender2 !== botOwner || !text2) return;
        ...
        if (setting.customOptions) {
          ...
          if (index >= 0 && index < setting.customOptions.length) {
            ...
            await reply("❌ Invalid number. Try again.");
          }
        } else {
          if (setting.key === "ANTIDELETE") {
            const enabled = input === "1";
            await setAnti(enabled);
            await reply(`✅ ${setting.label} set to *${enabled ? "ON" : "OFF"}*`);
            conn.ev.off("messages.upsert", toggleListener);
          } else {
            if (input === "1") {
...
} catch (err) { console.error("Settings command error:", err); } 
...
// ===== Commands from main.js added below =====
