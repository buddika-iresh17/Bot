...
conn.ev.on('messages.upsert', async (msg) => {
  try {
    ...
    const ownerNumber = global.owner.map(o => o[0]);
...
//================MAIN COMMAND================
  ...
  async function(conn, mek, m, {
    from, quoted, body, isCmd, command, args, q, isGroup, sender, senderNumber, reply, isOwner
  }) {
    // ✅ Owner-only check
    if (!isOwner) return reply("Only the bot owner can use this command.");

    try {
      reply("♻️ Restarting MANISHA-MD Bot...");
      await sleep(1500); // optional delay before restart
